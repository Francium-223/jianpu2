# -*- coding: utf-8 -*-
"""把 最终输出 与 权威GT 并排、逐位对齐打印, 用眼睛核对 token 是否一致。
去掉GT的括号连线后按 token 顺序对齐(用 digit+octave 匹配), 打印 输出 vs GT。"""
import re
from difflib import SequenceMatcher
from token_json import token_to_json
def load(file):
    t=open(file,encoding="utf-8",errors="replace").read()
    m=re.search(r"\d+\s*音\s*\n?\s*(.+)$",t,re.S)
    return [x for x in (m.group(1) if m else t).split() if re.match(r"^[,']*[qsdh#b]*[,']*[1-7x0-][.,'qsdh#b-]*$",x)]
# 权威GT
gts=[l.strip() for l in open("train-work/gt/春天在哪里.txt",encoding="utf-8").read().splitlines()]
gtl=[l for l in gts if re.match(r"^[qsdh,']*[0-9x,\-()'qsdh ]",l)]
gs=" ".join(gtl).replace("("," ").replace(")"," ")
GT=[x for x in gs.split() if re.match(r"^[,']*[qsdh#b]*[,']*[1-7x0-][.,'qsdh#b-]*$",x) or x=="-"]
OUT=load("train-work/spring_verify_final_out.txt")
def key(t):
    j=token_to_json(t); return (j["digit"],j["low"],len(j["voice"]),j["beam"],j["dotted"],j["accidental"])
jo=[key(x) for x in OUT]; jg=[key(x) for x in GT]
sm=SequenceMatcher(None,[(k[0],k[1],k[2]) for k in jo],[(k[0],k[1],k[2]) for k in jg])
print(f"输出 {len(OUT)} 音 | GT {len(GT)} 音\n")
print("=== 逐位对齐 (已忽略逗号/撇号大小写, 只看 digit+low+voice) ===")
align=[]
for tag,i1,i2,j1,j2 in sm.get_opcodes():
    if tag=="equal":
        for k in range(i2-i1): align.append((OUT[i1+k],GT[j1+k],"="))
    elif tag=="replace":
        for k in range(max(i2-i1,j2-j1)):
            oi=i1+k if i1+k<i2 else None; gj=j1+k if j1+k<j2 else None
            if oi is not None and gj is not None: align.append((OUT[oi],GT[gj],"X"))
            elif oi is not None: align.append((OUT[oi],"--","+"))
            else: align.append(("--",GT[gj],"-"))
    elif tag=="delete":
        for k in range(i2-i1): align.append((OUT[i1+k],"--","+"))
    elif tag=="insert":
        for k in range(j2-j1): align.append(("--",GT[j1+k],"-"))
pos=0
for o,g,m in align:
    print(f"{pos:3d}  [{m}] 输出[{o or '--':6s}]  GT[{g or '--':6s}]")
    pos+=1
