...
def report(name,file):
    ...
    print(f"{name:26s}: OK={ok:3d} 错{dif:3d} 多{dl:2d} 缺{ins:2d}")
for n,f in [("改前(rest0+0.60门)","train-work/spring_final_out.txt"),("改后(+low-clamp)","train-work/spring_lowclamp_out.txt")]:
    report(n,f)
