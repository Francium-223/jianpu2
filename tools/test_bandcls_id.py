...
page = max(glob.glob(os.path.join(hits[0], "*.jpg")), key=os.path.getsize)
...
for s0, e0 in T.fine_rows(content, T.ROW_GAP):
    for s, e in T.split_row_inner(content, s0, e0):
        ...
        if acc and fr < 0.85:
            items.append((s, e, fr, len(tl), nl, arr[s:e + 1]))
print(f"{os.path.basename(hits[0])[:40]}  可疑行带 {len(items)} 个")
flags = JP._classify_bands([it[5] for it in items])
for (s, e, fr, nt, nl, g), f in zip(items, flags):
    print(f"  y={s:4d}-{e:4d} frac={fr:.2f} tall={nt:3d} 横线={nl:2d} -> {'音符' if f else '文字'}")
